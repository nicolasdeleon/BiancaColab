...
def BarEvents_alldetail_view(request):
    #lista de todos los eventos que tenemos:
    qs = BarPost.objects.all()#obtengo todos los post
    template_name = 'BarEvento/alldetail_view.html' #esto lo tengo que asociar a un html creo
...
#retrieve one
@staff_member_required 
def BarEvento_event_view(request,slug):
    #lista de todos los eventos que tenemos:
    obj = get_object_or_404(BarPost,slug = slug) #obtengo el post del bar cuyo slug es unico
    template_name = 'BarEvento/event_view.html' #esto lo tengo que asociar a un html creo 
    users = obj.users.all()      
    context = {
        "title" : obj.title,
        "object" : obj,
...
def blog_post_update_view(request,slug):
    ...
    if form.is_valid():
        form.save()
    template_name = "BarEvento/updateEvento.html"
    context = {
        'title' : "update event ",
        'object' : obj,
        'form' : form,
...
def blog_post_delete_view(request,slug):
    ...
    if request.method == "POST":
        obj.delete()
        return redirect("/BarEvento/") 
    template_name = "BarEvento/deleteEvento.html"
    context = {
        'title' : "delete event ",
        'object' : obj,
    }
